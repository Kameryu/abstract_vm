...
int					parse(std::string str, Env *env, int cnt)
{
	...
	{
		if (!env->isExit())
			throw NoExitException("Missing exit instruction before leaving execution at line " + std::to_string(cnt));
	...
	if (!(std::regex_match(str, base_val) || std::regex_match(str, base_end)))
		throw ParsingErrorException("Line " + std::to_string(cnt) + " is not parsed as expected.");
	if (std::regex_match(str, base_val))
		inst_val(str, env);
	else
		inst_none(str, env);
	return 1;
